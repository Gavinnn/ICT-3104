...
<?php
//Query to select infoID
$id = $_GET['id'];
$record = DB::queryFirstRow("SELECT * FROM info WHERE infoID=%s", $id);
if (!$record) {
...
<html lang="en">
    <head>
        <!-- Basic -->
        <title>GymLife | Edit Content</title>
        <!-- Define Charset -->
        <meta charset="utf-8">
        <!-- Responsive Metatag -->
        ...
        <script>
            ...
            if (description == "" || description == null)
                    ...
                    $.ajax({
                        url: "editContentProcess.php",
                        data: {'description': description, 'infoID':infoID},
                        type: 'POST',
                        async: false,
                        success: function (data) {
                            if (data == "success") {
                                check = true;
                                successModal("Updated Successfully", "content.php");
                            } 
                        }
                    });
                }//end else
        ...
        <section id="about-section" class="about-section">
            <div class="container">
                <div class="row" style="margin-left:10px">
                    <form action='editContentProcess.php' method='post'  enctype='multipart/form-data' name='createreq-form' id='createreq-form'> 
                        <input type="hidden" id="infoID" name="infoID" value="<?php print $id; ?>"/>
